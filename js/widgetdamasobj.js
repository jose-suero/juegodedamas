...
var boardUtils = {
	...
	getCheckerInfo: function (checker) {
	    ...
	    return {
	        "coord": (whereiam) ? Coord.getCoordFromCell(whereiam) : null,
	...
	setCellOcupied: function (cell, ocupied, checker) {
	    var elem = $(cell);
	    var thisWidget = boardUtils.getWidget(elem);
	...
	checkerDropped: function(event, ui) {
		...
		if (Math.abs(cellAfter.data("boardRow") - cellBefore.data("boardRow")) == 2) {
			...
			if (cellBetween) {
				...
				if (checkerBetweenData && checkerBetweenData.type != ci.type) {
					...
					var piezasContrarias = $.grep(thisWidget.checkers, function(checker, index){
					    return (checker.data("whereiam") && checker.data("type") != ci.type);
					});
					if (piezasContrarias.length == 0) {
					    alert((ci.type == thisWidget._myInternalKeys.player1key ? thisWidget.options.player1.name : thisWidget.options.player2.name) + " Ganó");
					}
	...
	startDrag: function (event, ui) {
	    ...
	    if (boardUtils.currentPlayerHasCaptures(ui.helper)
	        ...
	        //bounce all capture checkers 
	        var thisWidget = boardUtils.getWidget(ui.helper);
	...
	checkerHasCaptures: function(checker) {
		...
		$(checkerMoves).each(function (index, move) {
            //Se pregunta capturesCount < 1, para saltar la evaluación en las otras interacciones.
		    if (capturesCount < 1 && move.type == "capture") capturesCount++;
		});
	...
	checkerPossibleMoves: function(checker) {
        ...
        //Si la coordenada no está correctamente establecida se retorna un arreglo vacío de inmediato.
		if (ci.coord == null) {
		    return [];
		}
        ...
        //Si la variable captures tiene un registro o más se devuelve dicha variable.
		return (captures && captures.length > 0) ? captures : result;
	}
